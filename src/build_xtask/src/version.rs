...
pub fn version() -> String {
    let manifest_path = vfs::workspace_dir().join("Cargo.toml");
...
fn bump_rpm_spec_version(new_version: &str, is_nightly: bool) {
    let spec_path = vfs::workspace_dir().join("packaging/rpm/vors.spec");
    ...
    let spec = {
        if is_nightly {
            ...
            // Grab version (ex: https://github.com/vors-org/VORS/archive/refs/tags/v16.0.0-rc1.tar.gz)
...
fn bump_deb_control_version(new_version: &str) {
    let control_path = vfs::workspace_dir().join("packaging/deb/control");
