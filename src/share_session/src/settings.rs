...
use settings_schema::{
    DictionaryDefault, OptionalDefault, SettingsSchema, Switch, SwitchDefault,
};
...
pub enum MicrophoneDevicesConfig {
    ...
    VoiceMeeterVaio3,
    Custom {
        #[schema(strings(help = "This device is used by VORS to output microphone audio"))]
...
pub enum DriverLaunchAction {
    ...
    #[schema(strings(display_name = "Unregister VORS at shutdown"))]
    UnregisterVorsAtShutdown,
    NoAction,
}
...
Unregister VORS at shutdown: This should be used when you want to load other drivers like for full body tracking. Other VR streaming drivers like Virtual Desktop must be manually unregistered or uninstalled.
No action: All driver registration actions should be performed manually, VORS included. This allows to launch SteamVR without launching the dashboard first."#
    ))]
